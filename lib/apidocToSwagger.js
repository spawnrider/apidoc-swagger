...
function toSwagger(apidocJson, projectJson) {
  ...
  // Add OpenAPI specifi data
  _.extend(swagger, projectJson.additionalData);
  return swagger;
}
...
function createSuccessResults(verbs, definitions) {
  ...
  var ret = _.mapValues(verbs.success ? verbs.success.fields : [], function (success) {
    var result = {
      "description": "Success"
    };
    if (success.length > 0 && success[0].field && success[0].field != 'null' && success[0].type && success[0].type != 'null') {
  ...
  // Fix default 200 return code (apidoc-core)
  if (ret['Success 200']) {
    ret['200'] = ret['Success 200'];
    delete ret['Success 200'];
  }
